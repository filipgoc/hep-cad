# CAO - Conception Assisté par Ordinateur I
{:.no_toc}

- [Session 01](cao_fr.md) `¯\_(ツ)_/¯`
- [Session 02](cao_2_fr.md)
- Session 03
- Session 04

---

## Table des matières
{:.no_toc}

* TOC
{:toc}

---

## Utilizer les Fichiers et Videos

<iframe width="960" height="600" src="https://www.youtube.com/embed/gyIphPyLjp4?rel=0" frameborder="0" allowfullscreen></iframe>

## Liens utiles

- [Raccourcis clavier. Graphique et agréable!](http://help.autodesk.com/view/fusion360/ENU/?guid=GUID-F0491540-0324-470A-B651-2238D0EFAC30)

[![](https://i.imgur.com/rcmINk2.png)](http://help.autodesk.com/view/fusion360/ENU/?guid=GUID-F0491540-0324-470A-B651-2238D0EFAC30)


## Sketcher & contraintes

![](https://i.imgur.com/NLDsMyi.png)

Le dessin dans Fusion360 n&#39;est pas comme Illustrator ou Inkscape. Il s&#39;agit ici de paramètres - variables. Dans les croquis, cela se traduit par des contraintes (choses comme `parallèle`,`angle droit&#39`,`5 mm de long`...)

Cela rend l&#39;esquisse un peu plus longue et moins naturelle, mais elle nous permet de réaliser des dessins précis et faciles à modifier.

Examinons certaines contraintes et des outils d&#39;esquisse en même temps.

Vous pouvez voir le fichier fini dans le dossier &quot;Screencasts&quot; dans notre hub Fusion360 partagé.

### Contraintes Exploration 1

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/1c4e7c48-5884-4550-8d6c-17d601f564b9" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### Contraintes Exploration 2

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/922d934c-b36b-4047-8004-7d41178469c5" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### Contraintes Exploration 3

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/f981efd7-1b09-45cf-89f7-13be515d8954" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### Contraintes Exploration 4

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/94b9c8da-d535-49d7-9120-705d85ce3d7a" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

## Outils de Modélisation de Base

![](http://cl.ly/1R0t272M0e3k/Image%202017-09-21%20at%209.28.43%20PM.png)

Faisons un simple cylindre de **7 manières différentes** - explorant ainsi les **outils de base** du modelisation en 3D.

**Outils de Modélisation de Base**

  1. **Insert a Solid - Insérez un solide**
  2. **Extrude - Extruder**
  3. **Loft**
  4. **Mirror - Mirroire**
  5. **Sweep - Balayage**
  6. **Revolve - Tourner**
  7. **Pattern (Circular) - Modèle**

Vous pouvez voir le fichier fini dans notre hub Fusion360 partagé. (N&#39;oubliez pas que vous pouvez voir l&#39;historique de la façon dont les cylindres ont été fabriqués. Sans prix lorsque vous ne savez pas quoi faire.)

### 1 Insert a Solid - Insérez un solide

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/701aaec4-e059-40e0-9739-72eded1af73d" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### 2 Extrude - Extruder

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/67d86372-4d31-43d8-b1a6-05467fdb779e" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### 3 Loft

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/a3973a02-7402-4772-8334-be0d829ac42b" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### 4 Mirror - Mirroiregrenier

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/f1e0a71e-4e9a-4fe3-8005-efabbf2a9948" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### 5 Sweep - Balayage

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/4b4cc6ae-93f0-47e1-b94f-d3a7c6bef5ea" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

### 6 Revolve - Tourner

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/ea29da31-bcd1-4bb7-a83d-8b252109da43" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

###  7 Pattern (Circular) - Modèle

<iframe width="960" height="820" src="https://screencast.autodesk.com/Embed/Timeline/9f432ad1-3bdc-4aed-b01e-b08b07d08aa7" frameborder="0" allowfullscreen webkitallowfullscreen></iframe>

---

## EXERCISE: Clé

Regardez le modèle de clé dans le dossier &quot;Exercice&quot;. Utilisez le calendrier pour déterminer exactement comment il a été fait.

## EXERCISE: Vis

Modèlez une vis de deux manières différentes.

D'abord, faites un dessin orthographique simple à la main. Devinez les mesures.

Demandez à Filip et il vous donnera un boulon à tenir.

Aidez-moi: 
-Il existe un outil «thread».

## EXERCISE: Android

Regardez le modèle du robot Android dans le dossier &quot;Exercice&quot;.

Faites-en comme ça :-)